...
test_that("perfect cadence", {
  trans <- c(
    c(0, 4, 7) %>% new_pc_set %>% add(10) %>% encode_pc_set,
    c(0, 4, 7) %>% new_pc_set %>% add(3) %>% encode_pc_set
  )
  norm <- c(
    c(0, 4, 7) %>% new_pc_set %>% add(7) %>% encode_pc_set,
    c(0, 4, 7) %>% new_pc_set %>% add(0) %>% encode_pc_set
  )
  expect_equal(
